...
#    print(f"File name: {file_name}")
    ...
    # Find the next file lexicographically
    if file_name in files:
        index = files.index(file_name)
        next_file = files[(index + 1) % len(files)]
#        print(f"@open {os.path.join(dir_path, next_file)}")  # Command for Sioyek
        subprocess.run(["sioyek", os.path.join(dir_path, next_file)])
#    else:
